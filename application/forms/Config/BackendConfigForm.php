...
/**
 * Form for managing the connection to the Windows backend
 */
class BackendConfigForm extends ConfigForm
{
    ...
    public function isValid($formData)
    {
        ...
        if (($el = $this->getElement('skip_validation')) === null || ! $el->isChecked()) {
            ...
            if (! $this->isValidWindowsDbSchema($resourceConfig)) {
                if ($el === null) {
                    $this->addSkipValidationCheckbox();
                }
    ...
    public function isValidWindowsDbSchema($resourceConfig)
    {
        try {
            $db = ResourceFactory::createResource($resourceConfig);
            $db->select()->from('host_check_results', array('host_id'))->fetchOne();
        } catch (Exception $_) {
        ...
        $this->addElement(
            ...
            array(
                'description'   => $this->translate(
                    'Check this to not to validate the Windows schema of the chosen resource'
                ),
                'ignore'        => true,
